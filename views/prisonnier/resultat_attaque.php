...
if ($resultat === 'success') {
    // Dégrader l'état
    $nouvel_etat = max(0, $prisonnier['etat'] - 10);
    $updateStmt = $pdo->prepare("UPDATE prisonnier SET etat = ? WHERE id = ?");
    $updateStmt->execute([$nouvel_etat, $cible_id]);

    // Supprimer l'objet ciblé
    $deleteObj = $pdo->prepare("DELETE FROM objets_prisonniers WHERE prisonnier_id = ? AND id = ?");
    $deleteObj->execute([$cible_id, $objet_id]);

    // Notification
    $notifMessage = "Votre santé a été dégradée et un objet vous a été confisqué après une attaque.";
    $insertNotif = $pdo->prepare("INSERT INTO notifications (recipient_id, type, message, created_at, is_read) VALUES (?, 'sante_degradee', ?, NOW(), 0)");
    $insertNotif->execute([$prisonnier['utilisateur_id'], $notifMessage]);
}
?>
<!DOCTYPE html>
<html lang="fr">
...
<body>
    <div class="result-card <?= $resultat === 'success' ? 'success' : 'fail' ?>">
        <?php if ($resultat === 'success'): ?>
            ...
            <p class="etat">Santé du prisonnier abaissée à <strong><?= htmlspecialchars($nouvel_etat) ?></strong>.</p>
        <?php else: ?>
            <h1>❌ Échec !</h1>

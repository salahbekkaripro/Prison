...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($username) || empty($email) || empty($password) || empty($captcha) || empty($role) || empty($nom) || empty($prenom) || $age === 0) {
    ...
    } elseif (!in_array($role, ['chef','admin','prisonnier','cuisinier','gestionnaire'])) {
        ...
        if ($stmt->fetch()) {
            ...
            $insert = $pdo->prepare("
                INSERT INTO users (
                    username, email, password, created_at, role, status, age, nom, prenom
                ) VALUES (
                    ?, ?, ?, NOW(), ?, ?, ?, ?, ?
                )
            ");
            ...
            switch ($role) {
              ...
              case 'cuisinier':
                  $stmt = $pdo->prepare("INSERT INTO cuisinier (utilisateur_id, date_affectation, specialite)
                                         VALUES (?, CURDATE(), 'générale')");
                  $stmt->execute([$userId]);
                  break;
          
              case 'chef':
                  $stmt = $pdo->prepare("INSERT INTO chef (utilisateur_id, date_nomination)
                                         VALUES (?, CURDATE())");
                  $stmt->execute([$userId]);
                  break;
